...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const available = !(await findUserByUsername(raw.toLowerCase()));
    res.json({
      valid: true,
      available,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const row = normalized.includes("@") ? await findUserByEmail(normalized) : await findUserByUsername(normalized);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await setUserPin(req.userId, hashPassword(pin));
...
router.get(
  ...
  requireAuth,
  asyncHandler(async (req, res) => {
    res.json(await getPinLockoutStatus(req.userId));
  })
);
