...
router.get(
  ...
  asyncHandler(async (req, res) => {
    res.json({
      categories: await listBudgetCategories(req.userId),
      budgets: await listBudgets(req.userId),
    });
  })
);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    // any brand-new category this call just created.
    const updatedBudgets = await upsertBudgets(req.userId, budgets);
    res.json({
      categories: await listBudgetCategories(req.userId),
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    await deleteBudgetCategory(req.userId, category);
    res.json({
      categories: await listBudgetCategories(req.userId),
      budgets: await listBudgets(req.userId),
    });
