...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const account = await createAccount({ userId: req.userId, accountName: accountName.trim(), balance: balance ?? 0 });
...
router.get(
  "/:id",
  asyncHandler(async (req, res) => {
    const account = await getAccount(req.params.id, req.userId);
    if (!account) {
